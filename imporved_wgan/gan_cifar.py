...
CLASS_LABEL = 1 # load only one class. None for all classes
SIZE = 150 # use subset of the traing data. None for everything
...
# Dataset iterators
train_gen, dev_gen = lib.cifar10.load(BATCH_SIZE, data_dir=DATA_DIR, class_label=CLASS_LABEL, size=SIZE)
def inf_train_gen():
    while True:
...
some_realsamples = np.array( [] )
while some_realsamples.shape[ 0 ] < 128:
    for img, label in train_gen():
        if len( some_realsamples ) == 0:
            some_realsamples = img
        else:
            some_realsamples = np.concatenate( (some_realsamples, img), axis=0 )


lib.save_images.save_images(some_realsamples[ : 128 ].reshape((128, 3, 32, 32)), 'real_samples.png')
